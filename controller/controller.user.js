...
// router.get('/all', paginatedResults(ModelPerson), (req, res) => {
...
//             let count = await ModelPerson.count()
...
exports.deleteUser = useAsync(async (req, res) => {
    try {
        ...
        await ModelPerson.deleteOne({ _id: req.body.id })
...
exports.allUser = useAsync(async (req, res) => {
    ...
    try {
        ...
        const user = await ModelPerson.find()
...
exports.changePassword = useAsync(async (req, res) => {
    ...
    const user = await ModelPerson.findOne({ _id: req.body.id });
    ...
    try {
        ...
        if (!originalPassword) {
            ...
            await ModelPerson.updateOne({ _id: req.body.id }, { password: NewPassword }).then(async () => {
                const New = await ModelPerson.findOne({ _id: req.body.id });
...
//     await ModelPerson.updateOne({ _id: req.params.id }, { $set: { active: false } }).then(() => {
...
//         await ModelPerson.updateOne({ _id: id }, body).then(async () => {
//             let user = await ModelPerson.findOne({ _id: id })
...
//         const NewPassword = await bcrypt.hash(req.body.newPassword, 13)
//         await ModelPerson.updateOne({ email: req.body.email }, { $set: { password: NewPassword } })
//         res.status(200).json({
...
//     let user = await ModelPerson.findOne({ _id: req.body.id })
...
//         await ModelPerson.updateOne({ _id: req.body.id }, { pin: req.body.pin }).then(async () => {
//             return res.status(200).json({
...
//             await ModelPerson.updateOne({ _id: req.body.id }, { pin: newPin }).then(async () => {
//                 const Newuser = await ModelPerson.findOne({ _id: req.body.id });
//                 return res.status(200).json({
//                     msg: 'Account Setup Successfully ',
//                     user: Newuser,
//                     status: 200
...
//         await ModelPerson.updateOne({ _id: req.body.id }, { $set: { image: req.body.image } }).then(async () => {
//             let user = await ModelPerson.findOne({ _id: req.body.id })
//             return res.status(200).json({
...
//         await ModelPerson.updateOne({ _id: id }, { $set: { blocked: false } }).then(async () => {
//             return res.status(200).json({
...
//         await ModelPerson.updateOne({ _id: id }, { $set: { blocked: true } }).then(async () => {
//             return res.status(200).json({
...
//         await ModelPerson.updateOne({ _id: req.body.id }, { $set: { tiers: "three", indemnity: true } }).then(async () => {
//             return res.status(200).json({
...
//     const user = await ModelPerson.findOne({ wallet_ID: req.body.usertag });
...
//     try {
//         const user = await ModelPerson.findOne({ wallet_ID: req.params.id });
...
//         const user = await ModelPerson.findOne({ _id: req.body.userId });
...
//             ModelPerson.updateOne({ _id: req.body.userId }, { $set: { note: req.body.note ,  reason: req.body.reason} }).then(async () => {
//                 return res.status(200).json({
//                     msg: 'Successful',
//                     status: 200
//                 })
